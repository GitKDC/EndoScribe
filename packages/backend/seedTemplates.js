...
// ── Force re-seed flag ───────────────────────────────────────────────────
// Run with:  node seedTemplates.js --force
// This wipes the existing `templates` table before inserting the fresh set
// below. Without --force, seeding is skipped if templates already exist
// (this is what was causing stale/old template data to keep showing up).
...
if (fs.existsSync(migrationPath)) {
  ...
  db.exec(migration, (err) => {
    if (err) {
      ...
      if (FORCE_RESEED) {
        console.log("⚠️  --force passed: clearing existing templates...");
        db.run("DELETE FROM templates", [], (delErr) => {
          if (delErr) {
            console.error("❌ Error clearing templates:", delErr);
            db.close();
            process.exit(1);
          }
...
function seedTemplates() {
  const templates = [
    ...
    // ─────────────────────────────────────────────────────────────────────
    // Structure mirrors the reference image:
    //   [DILATATION] heading
    //     Esophagus:- Stricture at post cricoid area / dilated to 15mm
    //   [POST DILATATION] heading
    //     Esophagus / Stomach / Duodenal Cap / IInd Part of Duodenum: Normal
    //   Impression
    // ─────────────────────────────────────────────────────────────────────
    {
  ...
  db.get("SELECT COUNT(*) as count FROM templates", [], (err, result) => {
    ...
    if (result.count > 0) {
      console.log(
        `⚠️  Database already has ${result.count} templates. Skipping seed.\n` +
        `    To re-seed with the latest templates, run:\n` +
        `    node seedTemplates.js --force`
      );
      db.close();
      process.exit(0);
    }
